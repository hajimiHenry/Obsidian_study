...
下图把定义式里的 $P+t\mathbf e$ 画出来：从点 $P$ 出发，只沿单位方向 $\mathbf e$ 走；$t$ 是走的距离，$\Delta x,\Delta y$ 是这一步在两个坐标轴上的分量。
...
这说明方向导数只取决于方向 $\mathbf e$ 在梯度方向上的投影。夹角越小，增长越快；夹角为直角时，方向导数为 $0$。
...
下图把梯度场叠在等值线上看：灰色曲线是等值线，红色箭头是每一点的梯度方向。箭头与等值线垂直，并指向函数值增加的方向。
